...
class FDB(nn.Module):
    def __init__(self, channels):
        ...
        self.combined_amp = nn.Sequential(nn.Conv2d(channels, channels, 1, 1, 0), nn.LeakyReLU(0.1, inplace=False),
                                      nn.Conv2d(channels, channels, 1, 1, 0))
    ...
    def forward(self, vis, inf):
        _, C, H, W = inf.shape
        inf = torch.fft.rfft2(self.pre1(inf) + 1e-8)
        vis = torch.fft.rfft2(self.pre2(vis) + 1e-8)
        inf_amp = torch.abs(inf)
        inf_pha = torch.angle(inf)
        vis_amp = torch.abs(vis)
        vis_pha = torch.angle(vis)

        amp_fuse = self.amp_fuse(torch.cat([inf_amp, vis_amp], 1))
        pha_fuse = self.pha_fuse(torch.cat([inf_pha, vis_pha], 1))
        real = amp_fuse * torch.cos(pha_fuse) + 1e-8
        imag = amp_fuse * torch.sin(pha_fuse) + 1e-8
        out = torch.complex(real, imag) + 1e-8
        out = torch.abs(torch.fft.irfft2(out, s=(H, W)))
...
class FSIM(nn.Module):
    def __init__(self, channelin, channelout):
        ...
        self.attention = nn.Sequential(nn.Conv2d(channelout, channelout, kernel_size=3, stride=1, padding=1), nn.Sigmoid())
        ...
        self.gate = GM(channelin)
        self.frq = FDB(channelin)

    def forward(self, vis, inf, i, j):
        _, C, H, W = vis.size()

        frq = self.frq(vis, inf)
        atten = self.norm1(frq)
        atb1 = inf * self.attention(inf)
        fused_OneOrderSpa = atten * atb1
        ...
        atb2 = infraredReflash1 * atb1
        fused_twoOrderSpa = fused_OneOrderSpa * atb2
        ...
        atb3 = infraredReflash2 * atb1
        fused_threeOrderSpa = fused_twoOrderSpa * atb3
        ...
        atb4 = infraredReflash3 * atb1
        fused_fourOrderSpa = fused_threeOrderSpa * atb4
...
class GCIM(nn.Module):
    ...
    def forward(self, vis, inf, i, j):
        ...
        fused = fused_fourOrderCha * gate + vis

        return fused, inf


class FDHI(nn.Module):
    def __init__(self, channelin, channelout):
        super(FDHI, self).__init__()
        self.spatial = FSIM(channelin, channelout)
        self.channel = GCIM(channelin, channelout)
...
class FDHIFusion(nn.Module):
    def __init__(self, vis_channels=1, inf_channels=1,
                 n_feat=16):
        super(FDHIFusion, self).__init__()

        self.vis = FeatureExtract(vis_channels, n_feat)
        self.inf = FeatureExtract(inf_channels, n_feat)

        self.interaction1 = FDHI(channelin=n_feat, channelout=n_feat)
        self.interaction2 = FDHI(channelin=n_feat, channelout=n_feat)
        self.interaction3 = FDHI(channelin=n_feat, channelout=n_feat)
